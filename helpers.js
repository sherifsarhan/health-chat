...
class Helpers {
  ...
  cleanDateRange(dateRange) {
    ...
    if (dateRange.length > 1) {
      ...
      dateRange.forEach(date => {
        ...
        if (
          tempMonth > currMonth ||
          (tempMonth = currMonth && tempDate >= currDate)
        ) {
  ...
  handleDateTimeRange(session, dateRangeClean) {
    ...
    if (dateRangeClean.length === 1) {
      ...
      session.userData.availableTimeslots = this.getAvailableTimeslots(
        session,
        new Date(dateRangeClean[0].end)
      );
      ...
      if (isTimeslotAvailable) {
        session.beginDialog("askTimeForGivenDay", {
          avail: isTimeslotAvailable
        });
      } else {
        session.beginDialog("askTimeForGivenDay", {
          unavail: !isTimeslotAvailable
        });
      }
    }
  ...
  handleDateRange(session, dateRangeClean, timeRange, singleDate) {
    ...
    if (!singleDate) {
      ...
      dates = this.getDates(
        startDate < todayDate ? todayDate : startDate,
        new Date(dateRangeClean[0].end)
      );
    ...
    dates.forEach(date => {
      let endTime;
      if (timeRange) {
        let startTime = this.builder.EntityRecognizer.parseTime(
          timeRange.start
        );
  ...
  isTimeslotAvailable(session, requestedDate) {
    let apptDatePath =
      schedule.doctorsSchedule[session.userData.doctorType][
        requestedDate.getFullYear()
      ][requestedDate.getUTCMonth()][requestedDate.getUTCDate()][
        requestedDate.getHours()
      ];
    if (
      apptDatePath &&
      apptDatePath[requestedDate.getMinutes()] === "available"
    ) {
      return true;
    }
    return false;
        ...
        for (var minute in minutes) {
          if (apptHours[hour][minute] === "available") {
            ...
            if (endTime) {
              ...
              let inRange = this.isInTimeRange(requestedDate, endTime, {
                hour,
                minute
              });
              if (inRange) {
  ...
  isInTimeRange(start, end, time) {
    start =
      start instanceof Date
        ? start
        : this.builder.EntityRecognizer.parseTime(start);
    end =
  ...
  getAvailableDays(session, timeRange) {
    if (timeRange)
      session.userData.requestedDate = this.builder.EntityRecognizer.parseTime(
        timeRange.start
      );
    let requestedDate =
      session.userData.requestedDate instanceof Date
      ...
      for (let month in sched[year]) {
        for (var day in sched[year][month]) {
          ...
          if (new Date(requestedDate).setHours(0, 0, 0, 0) <= schedDateObj) {
              ...
              for (var hour in schedDay) {
                for (var minute in schedDay[hour]) {
                  if (schedDay[hour][minute] === "available") {
                    ...
                    let inRange = this.isInTimeRange(
                      timeRange.start,
                      timeRange.end,
                      { hour, minute }
                    );
            ...
            if (
              !timeRange &&
              schedDay[requestedDate.getHours()] &&
              schedDay[requestedDate.getHours()][requestedDate.getMinutes()] &&
              schedDay[requestedDate.getHours()][requestedDate.getMinutes()] ===
                "available"
            ) {
              let availableDay = new Date(
